 ...
 * 
 * 04-23-2022: rewritten it entirely from scratch because
 * the old code was awfully full of macros that are not even
 * required for it to work. It's purely written by me now. 
 */
#include <kctype.h>
#include <ksprintf.h>
#include <stdint.h>
...
size_t ksprintf(char *s, const char *fmt, ...)
{
    size_t p;
    va_list va;
    va_start(va, fmt);
    p = kvsnprintf(s, (size_t)(-1), fmt, va);
    va_end(va);
    return p;
}
...
{
    return kvsnprintf(s, (size_t)(-1), fmt, va);
}

size_t ksnprintf(char *s, size_t n, const char *fmt, ...)
{
    size_t p;
    va_list va;
    va_start(va, fmt);
    p = kvsnprintf(s, n, fmt, va);
    va_end(va);
    return p;
}
...
static size_t strtosize(const char *s, const char **out)
{
    size_t value = 0;
    *out = s;
    while(kisdigit(**out)) {
